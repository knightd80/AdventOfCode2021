...
# Scale to full size of cave (5 times bigger in each direction and each cell incremented by 1 on each increase)
# extend in x direction
tempcaves = []
for row in caves:
    temprow = row.copy()
    for i in range(4):
        for index2 in range(len(caves[0])):
            tempvalue = int(row[index2]) + i + 1
            while tempvalue > 9:
                tempvalue -= 9
            temprow.append(tempvalue)
    tempcaves.append(temprow)
caves = tempcaves.copy()
...
for i in range(4):
    for row in caves:
        temprow = row.copy()
        for index2 in range(len(caves[0])):
            tempvalue = int(temprow[index2]) + i + 1
            while tempvalue > 9:
                tempvalue -= 9
            temprow[index2] = tempvalue
        tempcaves.append(temprow)
caves = tempcaves.copy()
